...
- 然后从Permission Mark Database中查询`Session ID -> User Groups`的映射，然后从Permission Mark Database中查询`Path -> Resource Groups, Resource Mark`的映射，然后从Permission Mark Database中查询`User Groups, Method, Resource Groups -> Choice`的映射，然后查看查询结果：
  - 如果`Path -> Resource Groups, Resource Mark`的映射不存在，说明该链接无效。
